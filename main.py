    ...
    while True:
        ...
        for j in range(start_update, start_update+num_updates):
            for step in range(config.num_steps):
                ...
                reward[~idx_bomb_kill] *= 0.25

                ## punish not laying bombs for too long
                #idx_random_nobomb_thr = (times_since_bomb >= 20 + np.random.randint(0, 5))
                #reward[idx_random_nobomb_thr] -= 0.05

                ## small random reward for placing bombs
                #idx_random_bomb_reward = (times_since_bomb >= (10 + np.random.randint(0, 3))) * action.cpu()==5
                #reward[idx_random_bomb_reward] += 0.05

                ## punish repeating patterns (lead to draws)
                #b = rollouts.actions.squeeze()
                #pattern_len = 4
                #c = b[-pattern_len:,:]
                #d = b[:-pattern_len,:]
                #
                #z = [[(c[:,j]==d[i:(i+c.shape[0]),j]).all() for i in range(d.shape[0]-c.shape[0])] for j in range(b.shape[1])]
                #z = torch.as_tensor(z)
                #
                #idx_repeats = (z.sum(1) >= 2)[:,None] * (times_since_done >= pattern_len*3)
                #if idx_repeats.sum(): print(" > Punishing repeating behaviour...")
                #reward[idx_repeats] -= 0.25
